...
class Report extends Controller
{
    ...
    {
        $school = School::first();
        if (!$school) return $this->showError('Data Sekolah belum diisi');

        $academic = AcademicYear::with('teacher')->active()->first();
        if (!$academic) return $this->showError('Tahun Akademik aktif belum diset');
        ...
        if (!$teacherGrade) return $this->showError('Data Wali Kelas belum diatur untuk kelas ini');
    ...
    public function getData($id)
    {
        ...
        $student = Student::with('dataStudent')->find($id);
        if (!$student) return $this->showError('Data Siswa tidak ditemukan');
        if (!$student->dataStudent) return $this->showError('Data Detail Siswa (Data Student) belum diisi');
